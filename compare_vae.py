...
def print_table(runs: list[dict]) -> None:
    header = (
        ...
        f"{'KL':>9}{'KL share':>10}{'mean|mu|':>10}{'max|mu|':>9}"
    )
    print(header)
    print("-" * len(header))
    for run in runs:
        print(
            ...
            f"{run['reconstruction']:>10.1f}{run['kl']:>9.2f}"
            f"{run['kl_share']:>9.2%}{run['mean_abs_mu']:>10.2f}{run['max_abs_mu']:>9.2f}"
        )
    print("-" * len(header))
...
def plot_comparison(runs: list[dict], path: Path) -> None:
    ...
    axis.plot(betas, [r["kl_share"] for r in runs], "o-", color="tab:green")
    axis.set_title("KL as a share of the loss")
    axis.set_xlabel("beta")
    axis.set_xscale("symlog")
    axis.set_yscale("log")
    axis.grid(True, alpha=0.3)
